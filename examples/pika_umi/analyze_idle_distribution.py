...
def main() -> None:
    ...
    ap.add_argument(
        "--idle-mm", type=float, default=0.3, help="per-step linear displacement below this counts as idle (mm)"
    )
    ap.add_argument(
        "--idle-frac",
        type=float,
    ...
    print(
        f"{'':<8} {'idle%':>7} {'|d| p50 mm':>11} {'p90 mm':>8} "
    ...
    for arm in ("left", "right"):
        ...
        print(
            f"{arm:<8} {100.0 * float((d < args.idle_mm).mean()):>6.1f}% {pct(d, 50):>11.3f} "
    ...
    for arm in ("left", "right"):
        ...
        print(
            f"{arm:<8} {pct(cm, 50):>10.3f} {pct(cm, 90):>10.3f} {pct(om, 50):>10.3f} "
    ...
    for arm in ("left", "right"):
        ...
        if tot:
            print(
                f"  {arm:<6} {len(cm)}/{tot} = {100.0 * len(cm) / tot:.1f}%  "

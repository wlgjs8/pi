...
def _make_dataset(
    repo_id: str, root: pathlib.Path, img_shape: tuple[int, int, int] = (480, 640, 3), state_dim: int = 14
):
    import shutil
...
def main(
    ...
    train_ds = _make_dataset(train_repo_id, lerobot_home / train_repo_id, img_shape, state_dim)
    val_ds = _make_dataset(val_repo_id, lerobot_home / val_repo_id, img_shape, state_dim) if n_val > 0 else None

...
def _make_hold_frame(last_frame: dict, gripper_action: str, state_mode: str = "pose") -> dict:
    ...
    if gripper_action == "absolute":
        ...
        # Read it from the last ACTION (always 14-D, grip at 6/13) so this works for BOTH state modes —
        # the 12-D velocity state has no gripper dims (the old state[6]/[13] crashed for state_mode=velocity).

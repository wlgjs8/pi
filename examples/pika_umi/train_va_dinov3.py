...
class VAPolicy(nn.Module):
    def __init__(self, size: str, weights_dir: pathlib.Path, horizon: int, layers: int = 4,
                 head_mode: str = "flow", drop_path: float = 0.0, split_head: bool = False,
                 backbone_type: str = "dinov3", wm_head: bool = False, state_tokens: int = 0):
        super().__init__()
    ...
    def wm_forward(self, ctx, gt_actions):
        ...
        b = ctx.shape[0]
        q = self.wm_queries.expand(b, -1, -1) + \
            self.wm_act(gt_actions.reshape(b, -1).to(ctx.dtype)).unsqueeze(1)
    ...
    def encode(self, img_l, img_r, state):
        ...
        if self.backbone_type == "siglip":
            ...
            td = self.dino_proj(
                self.backbone2.forward_features((x - m) / s)["x_norm_patchtokens"])  # (2b, 196, 1152)
        ...
        if self.state_tokens > 0:
            ...
            idx = torch.clamp(torch.floor((state.float() + 1.0) / 2.0 * self.state_tokens).long(),
                              0, self.state_tokens - 1)
...
def build_batch(batch, dev, aug=None, no_state=False, img_norm="imagenet", rotaug_deg=0.0,
                grip_state_random=0.0):
...
def param_groups(model, base_lr, head_lr, decay=0.75):
    ...
    rest = [p for nm, p in model.named_parameters()
            if not (nm.startswith("backbone.") or nm.startswith("backbone2."))]
...
def main() -> None:
    ...
    ap.add_argument("--backbone", default="dinov3", choices=["dinov3", "siglip", "sigdino"],
    ...
    ap.add_argument("--lwd", type=float, default=0.75,
                    help="layer-wise lr decay for the backbone; 1.0 = flat (use for siglip)")
    ...
    ap.add_argument("--row-decay", type=float, default=1.0,
    ...
    ap.add_argument("--exec-window", type=int, default=0,
    ...
    ap.add_argument("--distill-cache", type=str, default=None,
    ...
    ap.add_argument("--wm-cache", type=str, default=None,
    ...
    ap.add_argument("--wm-k", type=int, default=16,
                    help="lookahead in frames (30 Hz; 16 = 0.53 s, the L12-23 territory where the "
                         "gap lives)")
    ap.add_argument("--wm-weight", type=float, default=0.25)
    ap.add_argument("--state-tokens", type=int, default=0,
    ...
    ap.add_argument("--grip-state-random", type=float, default=0.0,
    ...
    ap.add_argument("--rotaug", type=float, default=0.0,
    ...
    ap.add_argument("--aug", default="none", choices=["none", "photo", "photo_geo"],
    ...
    ap.add_argument("--head", default="flow", choices=["flow", "l2", "dual"],
    ...
    if args.resolution != 224:
        ...
        new_inputs = tuple(
            _tf.ResizeImages(args.resolution, args.resolution, pad=t.pad)
            if isinstance(t, _tf.ResizeImages) else t
        ...
        dc = dataclasses.replace(dc, model_transforms=dataclasses.replace(
            dc.model_transforms, inputs=new_inputs))
    ...
    print(f"dataset {len(ds)} samples, horizon {H_data}"
          + (f" -> model horizon {H} (rows 0..{H - 1})" if H != H_data else ""), flush=True)
    ...
    if args.wm_cache:
        ...
        print(f"wm cache: {int(valid.sum())}/{len(ds)} frames usable at K={args.wm_k}, "
              f"latent dim {la.shape[2]}", flush=True)
    ...
    if args.grasp_weight > 1.0:
        ...
        _ps = _ilu.module_from_spec(_sp); sys.modules["ps"] = _ps; _sp.loader.exec_module(_ps)
        ...
        _f = list(_lds.episode_data_index["from"]); _t = list(_lds.episode_data_index["to"])
        w = np.ones(len(ds), dtype=np.float32)
        n_ev = 0
        for _ei in range(len(_f)):
            _a, _b = int(_f[_ei]), int(_t[_ei]); _st = _S[_a:_b - 1]; _T = _st.shape[0]
            ...
            for _bk in ("b1", "b3"):
                ...
                if 0 <= _bf < _T:
                    ...
                    n_ev += 1
        print(f"grasp-weight {args.grasp_weight} on {int((w > 1).sum())} frames "
              f"({100 * (w > 1).mean():.1f}%), {n_ev} events", flush=True)

        class _WeightedDS(torch.utils.data.Dataset):
            def __init__(self, base, weights): self.base, self.w = base, weights
            def __len__(self): return len(self.base)
            def __getitem__(self, i):
                it = dict(self.base[i]); it["_w"] = np.float32(self.w[i]); return it
        ds = _WeightedDS(ds, w)

    if teacher is not None:
        class _TeacherDS(torch.utils.data.Dataset):
            def __init__(self, base, t): self.base, (self.ta, self.tm, self.th) = base, t
            def __len__(self): return len(self.base)
            def __getitem__(self, i):
                it = dict(self.base[i])
    ...
    if wm is not None:
        class _WMDS(torch.utils.data.Dataset):
            def __init__(self, base, w): self.base, (self.la, self.va, self.fi) = base, w
            def __len__(self): return len(self.base)
            def __getitem__(self, i):
                it = dict(self.base[i])
                if bool(self.va[i]):
    ...
    if wm is not None:
        ...
        assert not args.rotaug, "wm latent targets are cached from UNROTATED frames; combining " \
                                "with --rotaug would pair rotated ctx with unrotated futures"
    model = VAPolicy(args.size, pathlib.Path(args.weights_dir), H, layers=args.layers,
                     head_mode=args.head, drop_path=args.drop_path,
                     split_head=args.split_head, backbone_type=args.backbone,
                     wm_head=wm is not None, state_tokens=args.state_tokens).to(dev).to(torch.bfloat16)
    if wm is not None:
        assert wm[0].shape[2] == model.wm_out.out_features, \
    ...
    row_w = None
    if args.exec_window:
        assert not args.row_decay != 1.0, "--exec-window and --row-decay both shape row_w; pick one"
        rw = np.ones(H, dtype=np.float32)
        rw[: min(args.exec_window, H)] = args.exec_weight
        rw = rw / rw.mean()
        row_w = torch.from_numpy(rw).to(dev).view(1, H, 1)
        print(f"exec-window {args.exec_window} @ w={args.exec_weight}: row0 {rw[0]:.2f} .. "
              f"row{H - 1} {rw[-1]:.2f} (executed rows carry "
              f"{100 * rw[:args.exec_window].sum() / rw.sum():.0f}% of the loss mass)", flush=True)
    ...
    opt = torch.optim.AdamW(param_groups(model, args.lr, args.head_lr, decay=args.lwd),
    ...
    while step < args.steps:
        for batch in loader:
            if step >= args.steps:
                break
            il, ir, st, ac, pad = build_batch(batch, dev, aug, args.no_state, img_norm, args.rotaug,
                                              args.grip_state_random)
            ...
            if args.head in ("flow", "dual"):
                ...
                t = (torch.distributions.Beta(1.5, 1.0)
                     .sample((ac.shape[0],)).to(dev) * 0.999 + 0.001)
                te = t[:, None, None]
            ...
            if step % 100 == 0:
                ...
                print(f"step {step:6d}/{args.steps}  loss {run / 100:.5f}  "
                      f"{dt / 100 * 1000:.0f} ms/it  lr {sched.get_last_lr()[-1]:.2e}", flush=True)
                run, t0 = 0.0, time.perf_counter()
            if step % args.save_every == 0 or step == args.steps:
                p = out / f"step_{step}.pt"
                torch.save({"model": model.state_dict(), "step": step, "size": args.size,
                            "horizon": H, "config": args.config, "head": args.head,
                            "aug": args.aug,
                            # eval/serving should read EMA, matching what pi05 checkpoints contain
                            "no_state": args.no_state, "resolution": args.resolution, "grasp_weight": args.grasp_weight, "drop_path": args.drop_path, "distill": bool(args.distill_cache),
                            "layers": args.layers, "split_head": args.split_head,
                            "row_decay": args.row_decay,
                            "exec_window": args.exec_window, "exec_weight": args.exec_weight,
                            "backbone": args.backbone, "weight_decay": args.weight_decay,
                            "lwd": args.lwd,
                            "wm": wm is not None, "wm_k": args.wm_k, "wm_weight": args.wm_weight,
                            "rotaug": args.rotaug, "grip_state_random": args.grip_state_random,
                            "state_tokens": args.state_tokens,
                            "ema": ({k: v.to(torch.bfloat16) for k, v in ema.items()}
                                    if ema is not None else None)}, p)

...
CONFIG=pi05_pika_umi_video_tcp_gripabs_velproprio_depth_h24
CKPT_BASE=/home/plaif/workspace/openpi_runs/checkpoints/${CONFIG}/depth_velproprio_gripabs_h24
VAL=plaif/pika_umi_video_val_tcp_gripabs_velproprio_depth
TRAIN=plaif/pika_umi_video_train_tcp_gripabs_velproprio_depth
...
for STEP in 10000 20000 30000 40000; do
  ...
  [ -f "$OUT/step${STEP}_depthvel_val${STEP}.json" ]   || run_eval "$STEP" "$VAL"   "depthvel_val${STEP}"   ""
  [ -f "$OUT/step${STEP}_depthvel_train${STEP}.json" ] || run_eval "$STEP" "$TRAIN" "depthvel_train${STEP}" "--limit 86"
done
